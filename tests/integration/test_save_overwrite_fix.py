...
def test_变更间隔秒数不会覆盖其他字段():
    ...
    task = 定时任务.objects.create(
        ...
        激活=True,
        间隔秒=60,
        两次最小间隔秒=600,  # 初始值 10 分钟
        设定时间=timezone.now()
    )
    
    # 模拟：用户在前端修改了两次最小间隔秒为 1200（20分钟）
    # 直接修改数据库（不通过模型实例）
    定时任务.objects.filter(id=task.id).update(两次最小间隔秒=1200)
    
    # 刷新任务对象（但保持旧值在内存中）
    task.refresh_from_db(fields=['间隔秒'])  # 只刷新间隔秒字段
    
    # 验证内存中的两次最小间隔秒仍是旧值 600
    assert task.两次最小间隔秒 == 600, f"期望值 600，实际 {task.两次最小间隔秒}"
    ...
    # 验证：两次最小间隔秒没有被覆盖（仍然是 1200，不是内存中的 600）
    assert task_refreshed.两次最小间隔秒 == 1200, \
        f"两次最小间隔秒期望值 1200，实际 {task_refreshed.两次最小间隔秒}"
    
    # 清理
    task.delete()
    print("✅ 测试通过：变更间隔秒数不会覆盖两次最小间隔秒")


@pytest.mark.django_db
def test_step方法不会覆盖其他字段():
    """测试 step 方法只更新 update_time 和 激活 字段"""
    print("\n=== 测试：step 方法不会覆盖两次最小间隔秒 ===")
    ...
    # 清理
    task.delete()
    print("✅ 测试通过：step 方法不会覆盖两次最小间隔秒")


@pytest.mark.django_db
def test_写入数据记录字典不会覆盖其他字段():
    """测试 写入数据记录字典 方法只更新数据字段"""
    print("\n=== 测试：写入数据记录字典不会覆盖两次最小间隔秒 ===")
    ...
    # 清理
    task.delete()
    print("✅ 测试通过：写入数据记录字典不会覆盖两次最小间隔秒")
