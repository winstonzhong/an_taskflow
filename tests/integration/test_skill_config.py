...
class TestSkillConfigSave:
    ...
    def test_save_model_field_config(self):
        """
        【新增】测试保存模型字段配置（如 两次运行最小间隔秒数）
        
        场景：
        - 保存包含 两次运行最小间隔秒数 的配置
        - 验证 两次运行最小间隔秒数 被保存到模型字段，而不是配置 JSON
        """
        # 准备：创建任务，设置初始两次运行最小间隔秒数
        task = 定时任务.objects.create(
            名称="模型字段测试任务",
            group_name="模型字段测试技能",
            激活=True,
            两次运行最小间隔秒数=300,  # 初始值 5 分钟
            配置={},
            设定时间=timezone.now()
        )
        ...
        # 验证：两次运行最小间隔秒数被更新到模型字段
        task.refresh_from_db()
        assert task.两次运行最小间隔秒数 == 600  # 验证模型字段被更新
        
        # 验证：普通配置项保存到配置 JSON
        assert task.配置 == {"普通配置项": "普通值"}
        
        print("✅ 模型字段（两次运行最小间隔秒数）保存成功")
...
class TestSkillConfigIntegration:
    ...
    def test_save_and_get_config_consistency(self):
        ...
        assert len(model_keys) >= 1  # 至少包含 两次运行最小间隔秒数
        interval_key = next((k for k in model_keys if k['name'] == '两次运行最小间隔秒数'), None)
        assert interval_key is not None
        # 两次运行最小间隔秒数的值直接从模型字段读取（使用默认值 600）
...
class TestSkillConfigDuplicateHandling:
    ...
    def test_model_field_overrides_json_config(self):
        ...
        场景：
        - JSON 配置中有 "两次运行最小间隔秒数" 这个 key
        - MODEL_CONFIG_FIELDS 也有 "两次运行最小间隔秒数"
        - 验证只展示模型字段的值，JSON 中的被忽略
        """
        client = Client()
        skill_name = "重复配置处理测试"
        
        # 准备：创建任务，JSON 配置中有 "两次运行最小间隔秒数"，同时模型字段也有值
        task = 定时任务.objects.create(
            名称="重复配置任务",
            group_name=skill_name,
            激活=True,
            两次运行最小间隔秒数=600,  # 模型字段值
            配置={
                "两次运行最小间隔秒数": 300,  # JSON 配置值（应该被忽略）
                "普通配置": "应该显示"
            },
            设定时间=timezone.now()
        )
        ...
        # 验证只返回一个 "两次运行最小间隔秒数"（模型字段的）
        interval_keys = [k for k in keys if k['name'] == '两次运行最小间隔秒数']
    ...
    def test_save_with_duplicate_config_name(self):
        ...
        # 验证模型字段被更新
        assert task.两次运行最小间隔秒数 == 900
        
        # 验证 JSON 配置中没有 "两次运行最小间隔秒数"（因为被提取出来保存到模型字段了）
        assert "两次运行最小间隔秒数" not in task.配置
    ...
    def test_legacy_name_filtered(self):
        ...
        场景：
        - 数据库中 JSON 配置使用旧名称 "两次运行最小间隔秒"
        - MODEL_CONFIG_FIELDS 使用新名称 "两次运行最小间隔秒数"
        - 验证只显示新名称的配置，旧名称被过滤掉
        """
        client = Client()
        skill_name = "旧名称过滤测试"
        ...
        # 验证旧名称 "两次运行最小间隔秒" 没有出现在结果中
        legacy_keys = [k for k in keys if k['name'] == '两次运行最小间隔秒']
    ...
    def test_legacy_name_fallback(self):
        ...
        interval_key = next((k for k in keys if k['name'] == '两次运行最小间隔秒数'), None)
        assert interval_key is not None
...
class TestModelFieldFallback:
    ...
    def test_model_field_zero_fallback_to_json(self):
        ...
        task = 定时任务.objects.create(
            ...
            group_name=skill_name,
            激活=True,
            两次运行最小间隔秒数=0,  # 模型字段为 0
            配置={
                "两次运行最小间隔秒数": 550,  # JSON 配置值（应该被使用）
            },
            设定时间=timezone.now()
        )
    ...
    def test_model_field_effective_value_no_fallback(self):
        ...
        task = 定时任务.objects.create(
            ...
            group_name=skill_name,
            激活=True,
            两次运行最小间隔秒数=750,  # 模型字段有有效值
            配置={
                "两次运行最小间隔秒数": 250,  # JSON 配置值（应该被忽略）
            },
            设定时间=timezone.now()
        )
